...
export default function WorkflowsRuns() {
  ...
  const { data, isLoading, refetch } = useData<{ runs: WorkflowRun[]; total: number }>('/v1/runs', {
  ...
  return (
    <UIPage width="full">
      ...
      {/* View */}
      {viewMode === 'triage' && (
        <TriageView runs={runs} onRunClick={(id) => navigate(`/workflows/runs/${id}`)} />
      )}
      {viewMode === 'board' && (
        <BoardView runs={runs} onRunClick={(id) => navigate(`/workflows/runs/${id}`)} />
      )}
      {viewMode === 'table' && (
        <UICard>
          <TableView runs={runs} loading={isLoading} onRunClick={(id) => navigate(`/workflows/runs/${id}`)} />
        </UICard>
      )}
    </UIPage>
  );
